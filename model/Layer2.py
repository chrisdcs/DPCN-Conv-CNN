...
class Layer2(nn.Module):
    ...
    def __init__(self, N:int, K:int, S:int, R_lr:float=0.1, lmda:float=5e-3, device=None, save_dir=None):
        ...
        self.B = nn.ConvTranspose2d(m2, m1, kernel_size=ksize, stride=kstride, padding=ksize//2, bias=False).to(device) #if no pooling kernel_size=ksize-1,stride=kstride*2, padding=1
    ...
    def Fista_(self, img_batch, Rt_1,  td, t = 0, td_alpha = 0.01):
        ...
        for m in range(300):
            ...
            while keep_going and protect_x<20000:#torch.sum(stop_linesearch) != batch_size:
                ...
                self.R.data = (torch.sign(gk)*(torch.max((torch.abs(gk)-gama/Lx),zero_tensor))).clone()#Layer1.soft_thresholding_(self.R, self.lmda)
                pred = self.U(self.R)
            ...
            for u_iter in range(1):
                ...
                self.sparse_u = np.maximum(0.98*self.sparse_u,1e-3)#0.97-100
                tk_1u = 1+((((m4u+1))**8-1)/2)
        ...
        if not self.save_dir:
            np.save('costx2.npy',lossx)
            np.save('costu2.npy',lossu)  
        else:
            np.save(self.save_dir + '/' + 'costx2.npy',lossx)
            np.save(self.save_dir + '/' + 'costu2.npy',lossu)
    ...
    def normalize_weights_U(self):
            #self.U.weight.data = self.U.weight.data/(torch.sqrt(torch.sum(self.U.weight.data**2,(2,3)))).unsqueeze(2).unsqueeze(3)
            #self.B.weight.data = self.B.weight.data/(torch.sqrt(torch.sum(self.B.weight.data**2,(2,3)))).unsqueeze(2).unsqueeze(3) 
        with torch.no_grad():
    ...
    #         self.B.weight.data = F.normalize(self.B.weight.data, dim=0)

    def forward(self, img_batch, x, u, xp, Rt_1):
        # first fit
        #self.Fista_(img_batch)
        # now predict again
        pred = self.U(x)

...
m1 = 768 #channels of states
m2 = 768 #channels of causes
ksize_x = 3 #kernel size
ksize_u = 3
kstride_x = 1 #kernel stride
kstride_u = 1

inputsize = 3 #channels of inputs
batch_size = 32
Dx = 64 #length and width of inputs

class Layer1(nn.Module):

    def __init__(self, N:int, K:int, S:int, R_lr:float=0.1, lmda:float=5e-3, device=None, save_dir = None): # ignore those parameters
        super(Layer1, self).__init__()
        self.N = N # ignore those parameters
        self.K = K # ignore those parameters
        self.S = S # ignore those parameters
        self.R_lr = R_lr # ignore those parameters
        self.lmda = lmda # ignore those parameters
        self.save_dir = save_dir

        self.sparse_u = 0.1 #0.1 # sparsity of cause u
        self.alphaA = 0.1  #alphaA is the coefficient of transition level

        self.device = torch.device("cpu") if device is None else device
        self.U = nn.ConvTranspose2d(m1, inputsize, kernel_size=ksize_x, stride=kstride_x, padding=ksize_x//2, bias=False).to(device) # filters of state: U
        torch.nn.init.uniform_(self.U.weight,-0.5,0.5)
        
        self.B = nn.ConvTranspose2d(m2, m1, kernel_size=ksize_u, stride=kstride_u, padding=ksize_u//2, bias=False).to(device) # filters of cause: B # if no pooling kernel_size=ksize-1,stride=kstride*2, padding=2
        torch.nn.init.uniform_(self.B.weight,0,1)
        
        self.A = nn.ConvTranspose2d(m1, m1, kernel_size=ksize_x, stride=1, padding=ksize_x//2, bias=False).to(device)# # filters of transition
        ...
        # responses
        self.R = None #state
        self.Rz = None #auxiliary state: state'
        self.u = None # cause
        self.uz = None # auxiliary cause: cause'
        self.xp = None # pooled state
        
        self.maxpool = nn.MaxPool2d(2, stride=2, return_indices=True)
        self.unpool = nn.MaxUnpool2d(2, stride=2)
        
        # initially 0.2
        self.lamb = 0.2#0.1 # sparsity of state R

        self.normalize_weights_U() # normalize U
        self.normalize_weights_B() # normalize B
        
        self.index = None

    def Fista_(self, img_batch, Rt_1, td, t = 0, td_alpha = 0.01): #td_alpha is the coefficient of top-down level, td is the predied causes from top layer
        batch_size = img_batch.shape[0]
        self.sparse_u = 0.2#0.2
        self.alphaA = 0.1 # alphaA is the coefficient of transition level for updating states, if large then states change little
        ## size check of causes and states##
        #Dx = img_batch.shape[2] 
        #assert (Dx - ksize_x) % kstride == 0, "Kernel and stride size mismatch"
        #cx = (Dx - ksize) // kstride + 1
        #Du = int(cx/2)
        #assert (Du - ksize) % kstride == 0, "Kernel and stride size mismatch"
        #cu = (Du - ksize) // kstride + 1
        
        # create states and causes
        if t == 0 :
            ...
            self.u = torch.zeros((img_batch.shape[0], m2, Dx//2, Dx//2), requires_grad=True, device=self.device)
            self.uz = torch.zeros((img_batch.shape[0], m2, Dx//2, Dx//2), requires_grad=True, device=self.device)
            #td_alpha = 0.0
            #self.alphaA = 0
        else:
        ...
        tku = 1
        tk_1u = 1
        mu = 0.01/(m1*Dx*Dx)#the parameter of Neov smoothing
        ...
        for m in range(300):
            ...
            while keep_going and protect_x<20000:#torch.sum(stop_linesearch) != batch_size: # search step size using lipschitz condition        
                ...
                self.R.data = (torch.sign(gk)*(torch.max((torch.abs(gk)-gama/Lx),zero_tensor))).clone() #soft thresholding
                pred = self.U(self.R)
            ...
            for u_iter in range(1):
                ...
                while keep_going and protect_u<20000:#torch.sum(stop_linesearch) != batch_size:# search step size using lipschitz condition
                    ...
                    self.u.data = (torch.sign(gk)*(torch.max((torch.abs(gk)-self.sparse_u/Lu),zero_tensor))).clone() #soft thresholding
                    ...
                    if (temp1.sum())<= (temp2.sum()):
                        keep_going = 0
                # initially 0.985
                self.sparse_u = np.maximum(0.985*self.sparse_u,1e-3)#0.975-100
        ...
        if not self.save_dir:
            np.save('costx1.npy',lossx)
            np.save('costu1.npy',lossu)   
        else:
            np.save(self.save_dir + '/' + 'costx1.npy',lossx)
            np.save(self.save_dir + '/' + 'costu1.npy',lossu)
        
        return self.R.data.detach(), self.u.data.detach(), self.xp.data.detach() #return state, cause, pooled state
